...
class GeneticAlgorithm:
    def __init__(self, configuration, numberOfChromosomes=100, replaceByGeneration=8, trackBest=5,
                 numberOfCrossoverPoints=2, mutationSize=10, crossoverProbability=80, mutationProbability=60, elitism_size=10):
    ...
    def replace_generation(self):
        # Apply elitism: Preserve the best chromosomes
        elite_chromosomes = sorted(self._chromosomes, key=self.fitness, reverse=True)[:self.elitism_size]
        ...
        # Ensure elite chromosomes are carried over
        self._chromosomes[:self.elitism_size] = elite_chromosomes
    ...
    def run(self, max_generations=1000, min_fitness=0.999):
        ...
        while current_generation < max_generations:
            ...
            if repeat_count > max_generations // 100:
                random.seed(round(time() * 1000))  # Reset the seed to escape local optima
                self.set_replace_by_generation(self._replaceByGeneration * 3)
                self._crossoverProbability = min(100, self._crossoverProbability + 10)
                self._mutationProbability = min(100, self._mutationProbability + 10)
    ...
    def fitness(self, schedule):
        ...
        session_counts_per_slot = [0, 0, 0]

        for workshop, slots in schedule.session_bookings.items():
            for slot, campers in slots.items():
                session_counts_per_slot[slot] += 1

                # Penalize if session exceeds max capacity
                if len(campers) > schedule.max_slots_per_workshop:
                    score -= (len(campers) - schedule.max_slots_per_workshop) * 2  # Strong penalty
                # Penalize if session has fewer than the minimum required campers
                elif len(campers) < schedule.min_slots_per_workshop:
                    score -= (schedule.min_slots_per_workshop - len(campers)) * 2  # Strong penalty
                else:
                    score += 3  # Reward valid session size

                # Penalize for age group mismatches within a session
                age_groups = {schedule.configuration['campers'][c]['age_group'] for c in campers}
                if len(age_groups) == 1:
                    score += 2  # Reward single age group sessions
                else:
                    score -= len(age_groups)  # Penalize for mixed age groups

        # Penalize for exceeding the max sessions per slot
        for slot in range(3):
            if session_counts_per_slot[slot] > schedule.max_sessions_per_slot:
                score -= (session_counts_per_slot[slot] - schedule.max_sessions_per_slot) * 2  # Strong penalty

        # Reward for unique workshop assignments for each camper
        for camper_id, workshops in schedule.schedule.items():
            if len(set([w for w, _ in workshops])) == len(workshops):
                unique_assignments += 1
            # Reward if camper gets at least one of their preferred workshops
            preferences = set(schedule.configuration['campers'][camper_id]['preferences'])
            fulfilled_preferences = sum(1 for workshop, _ in workshops if workshop in preferences)
            score += fulfilled_preferences
    ...
    def crossover(self, parent1, parent2):
        ...
        for i, camper_id in enumerate(parent1.schedule):
            ...
            for workshop, s in schedule_to_use:
                if s not in assigned_slots and len(child.session_bookings[workshop][s]) < child.max_slots_per_workshop:
...
#
# class GeneticAlgorithm:
#     def __init__(self, configuration, numberOfChromosomes=100, replaceByGeneration=2, trackBest=5,
#                  numberOfCrossoverPoints=2, mutationSize=5, crossoverProbability=80, mutationProbability=50, elitism_size=20):
#         self.configuration = configuration
#         self._chromosomes = [Schedule(configuration) for _ in range(numberOfChromosomes)]
#         self._bestFlags = [False] * numberOfChromosomes
#         self._bestChromosomes = [0] * trackBest
...
#         for workshop, slots in schedule.session_bookings.items():
#             for slot, campers in slots.items():
#                 # Only penalize for exceeding capacity
#                 if len(campers) <= schedule.max_slots_per_workshop:
#                     score += 1
#                 else:
#                     score -= (len(campers) - schedule.max_slots_per_workshop)
#         return score
#
#     def crossover(self, parent1, parent2):
#         if random() * 100 > self._crossoverProbability:
...
#                 schedule_to_use = parent2.schedule[camper_id] if camper_id in parent2.schedule else []
#
#             child.schedule[camper_id] = schedule_to_use
#             for workshop, slot in schedule_to_use:
...
#             preferences = schedule.configuration['campers'][camper_id]['preferences']
#
#             # Simplified mutation: Just reassign to a random valid workshop and slot
#             new_workshop = random.choice(preferences)
#             new_slot = random.randint(0, 2)
#
#             # Update session bookings
#             old_workshop = workshops[random.randint(0, len(workshops) - 1)]
#             old_workshop_name = old_workshop[0]
#             old_workshop_slot = old_workshop[1]
#
#             schedule.session_bookings[old_workshop_name][old_workshop_slot].remove(camper_id)
#             schedule.session_bookings[new_workshop][new_slot].append(camper_id)
#
#             # Update camper schedule
#             schedule.schedule[camper_id][random.randint(0, 2)] = (new_workshop, new_slot)
#
#     def calculate_satisfaction_rate(self, schedule):
#         satisfaction_counts = {0: 0, 1: 0, 2: 0, 3: 0}
#
